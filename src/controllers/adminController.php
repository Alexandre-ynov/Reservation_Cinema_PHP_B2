...
class AdminController {
	...
	public function addFilm() {
		...
		if ($data['filmId'] === '' || $data['filmTitle'] === '') {
			$_SESSION['admin_error'] = 'Film ID and title are required.';
			redirect_to('/admin');
		}
	...
	public function updateFilm() {
		...
		$success = $this->model->updateFilm($data);
		$_SESSION['admin_message'] = $success ? 'Film updated successfully.' : 'Failed to update film.';
		redirect_to('/admin');
	}
	...
	public function addSceance() {
		...
		if ($sceanceId === '' || $sceanceDateRaw === '' || $filmId === '' || $roomId === 0) {
			$_SESSION['admin_error'] = 'All sceance fields are required.';
			redirect_to('/admin');
		}
